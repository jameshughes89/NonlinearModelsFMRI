...
#TOPX = 30
for TOPX in range(2,31):

	lastsCount = 0

	for t in tasks:
		bestExpressions = []
		oFile = open('./topModels/bestExpressions-L-TOP' + str(TOPX) + '-' + t + '-RL.txt','w')	
		o2File = open('./topModels/bestExpressionsError-L-TOP' + str(TOPX) + '-' + t + '-RL.txt','w')	
		fs='funcsL = ['
		count = 0
		for s in subjects:
			try:		
